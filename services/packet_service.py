...
def get_interface_info(ifname, sock):
    ...
    ip_address = socket.inet_ntoa(fcntl.ioctl(sock.fileno(), 0x8915, ifr)[20:24])
...
def send_packet_logic(data):
    try:
        ...
        # Packet number logic
        num_packets = (len([c for c in config_list if c.get("status")]) - 1) // 100 + 1
        ...
        for packet_index in range(1, num_packets + 1):
            ...
            for config_item in config_list[(packet_index - 1) * 100:packet_index * 100]:
                if config_item.get("status", False):
                    mac = binascii.unhexlify(config_item["mac"].replace(":", ""))
            ...
            current_payload.extend([0x00] * (MTU_SIZE - 14 - 20 - 20 - len(current_payload)))

            # Construct Ethernet header
            eth_header = struct.pack("!6s6sH", dest_mac, src_mac, ETHER_TYPE_IPV4)
            ...
            pseudo_header = struct.pack(
                "!4s4sBBH", socket.inet_aton(src_ip), dest_ip, 0, socket.IPPROTO_TCP, 20 + len(current_payload)
            )
            ...
            tcp_checksum = checksum(pseudo_header + tcp_header + current_payload)
...
#         # Pad remaining payload to 1500 bytes
#         if len(payload) > 1446:
#             raise ValueError("Payload size exceeds MTU")
#         payload.extend([0x00] * (1446 - len(payload)))
...
#         window = socket.htons(5840)
#         pseudo_header = struct.pack(
#             "!4s4sBBH", socket.inet_aton(src_ip), dest_ip, 0, socket.IPPROTO_TCP, 20 + len(payload)
#         )
#         tcp_header = struct.pack(
#             "!HHLLBBHHH",
#             DEFAULT_SRC_PORT, DEFAULT_DEST_PORT, seq_num, 0,
